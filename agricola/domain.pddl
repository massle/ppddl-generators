...
;; probabilistic version adapted by Marcel Steinmetz (2025)
;;
(define (domain agricola)
(:requirements :typing :action-costs :negative-preconditions :probabilistic-effects)
(:types
...
(:action ag__harvest_collecting_veg
    :parameters (?r - round ?s - stage ?v - vegetable ?i1 ?i2 ?i3 ?i4 ?i5 - num )
    :precondition (and (hold_round ?r roundend)
                       (harvest_phase ?s harvest_init)
                       (category_round ?r tharvest)
                       (sown_veg ?v)
...
(:action ag__harvest_breeding_animal
    :parameters (?r - round ?s - stage ?a - animal ?i ?i2 ?i3 ?i4 - num)
    :precondition (and (hold_round ?r roundend)
                       (harvest_phase ?s harvest_breeding)
                       (category_round ?r tharvest)
                       (owned_animals ?a)
...
(:action take_food
    :parameters (?w1 ?w2 ?wmax - worker ?r - round ?i1 ?i2 ?i3 ?i4 - num)
...
(:action collect_cook_animal
    :parameters (?a - animal ?act - animaltag ?w1 ?w2 ?wmax - worker ?r - round ?i1 ?i2 ?i3 ?i4 - num)
    :precondition (and (available_action ?act)
                       (open_action ?act)
                       (current_worker ?w1)
                       (NEXT_WORKER ?w1 ?w2)
